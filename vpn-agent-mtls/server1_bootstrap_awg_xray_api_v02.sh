...
# Server-1 bootstrap v02: AmneziaWG (awg0) + dnsmasq DNS + Xray (VLESS/REALITY) transparent TCP redirect + awgctl CLI + nginx mTLS + vpn-agent
...
PY
python3 - /usr/local/bin/awgctl "${AWG_IF}" "${AWG_ADDR}" "${AWG_SERVER_IP}" "${SERVER_PUBLIC_IP}" <<'PY'
from pathlib import Path
import sys

path = Path(sys.argv[1])
awg_if, awg_addr, awg_server_ip, server_public_ip = sys.argv[2:6]
