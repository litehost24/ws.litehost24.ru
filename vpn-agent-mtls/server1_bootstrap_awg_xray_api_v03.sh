...
# Server-1 bootstrap v03: AmneziaWG (awg0) + dnsmasq DNS + Xray (VLESS/REALITY) transparent TCP redirect + awgctl CLI + nginx mTLS + vpn-agent
# IMPORTANT: Intended for fresh installs. If node state already exists, the script will stop unless CONFIRM_REWRITE=1.
#
# Usage (as root):
#   bash server1_bootstrap_awg_xray_api_v03.sh
#
...
if [[ "${AWG_QUICK_UNIT}" == awg-quick@* ]]; then
