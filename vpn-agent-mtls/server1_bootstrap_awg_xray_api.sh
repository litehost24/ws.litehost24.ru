...
# IMPORTANT: Intended for fresh installs. If Xray config already exists, the script will stop unless CONFIRM_REWRITE=1.
#
# Usage (as root):
...
#   CONFIRM_REWRITE=1 (required only if Xray configs already exist)
...
VLESS_UUID="${VLESS_UUID:-8072b066-3a46-4867-9559-c62eb83b39da}"
...
if [[ -n "${VLESS_BACKUP_ADDR}${VLESS_BACKUP_PORT}${VLESS_BACKUP_UUID}${VLESS_BACKUP_FLOW}${REALITY_BACKUP_SNI}${REALITY_BACKUP_PBK}${REALITY_BACKUP_SID}${REALITY_BACKUP_FP}${REALITY_BACKUP_SPX}" ]]; then
...
# Bring up now and enable on boot.
# Prefer awg-quick@ for AmneziaWG; fallback to wg-quick@ if awg-quick@ is unavailable.
if ip link show "${AWG_IF}" >/dev/null 2>&1; then
  awg-quick down "${AWG_IF}" || true
fi
awg-quick up "${AWG_IF}"
if systemctl list-unit-files | awk '{print $1}' | grep -qx 'awg-quick@.service'; then
  AWG_QUICK_UNIT="awg-quick@${AWG_IF}.service"
else
  AWG_QUICK_UNIT="wg-quick@${AWG_IF}.service"
fi
...
if [[ "${active}" == "primary" ]]; then
  if check_host "${PRIMARY_ADDR}" "${PRIMARY_PORT}"; then
    ...
    ok_count=0
    if (( fail_count >= FAIL_THRESHOLD )) && (( cooldown_ok )); then
      switch_to backup
      active="backup"
      fail_count=0
      ok_count=0
...
# Many browsers try HTTP/3 (QUIC, UDP/443). Block it so clients fall back to TCP which goes via Xray/VLESS.
...
def rewrite_client_config(config_text: str, rec: dict) -> str:
    ...
    for line in lines:
        ...
        if section == "interface" and stripped.startswith("Address ="):
            out.append(f"Address = {rec['ip']}")
            address_done = True
            continue
        if section == "peer" and stripped.startswith("AllowedIPs ="):
            out.append(f"AllowedIPs = {rec.get('allowed') or DEFAULT_ALLOWED}")
            allowed_done = True
            continue
